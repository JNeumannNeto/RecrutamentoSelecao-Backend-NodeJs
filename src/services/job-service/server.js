...
app.get('/health', (req, res) => {
  res.json({
    success: true,
    message: 'Job Service está funcionando',
...
const PORT = config.services.jobService.port;
...
const startServer = async () => {
  try {
    ...
    server = app.listen(PORT, () => {
      logger.info(`Job Service rodando na porta ${PORT}`);
    ...
    logger.error('Erro ao iniciar Job Service:', error);
    process.exit(1);
  }
};
...
const gracefulShutdown = async (signal) => {
  ...
  if (server) {
    server.close(async () => {
      ...
      logger.info('Job Service encerrado com sucesso');
      process.exit(0);
    });
  } else {
    process.exit(0);
